...
def find_contours(image):
    ...
    im2, con, hierarchy = cv2.findContours(filtered,
                                           cv2.RETR_TREE,
...
def prepare_data(im, c, points, x_c, y_c, shape_ref):
    ...
    similarity = np.float(cv2.matchShapes(c, shape_ref, cv2.CONTOURS_MATCH_I2,
                                          0))
    area = get_percentage_of_area(c)
    return np.concatenate((angles, ranges, hsv[0, 0, 1:], [similarity],
                           [area]))


def get_ranges_and_angles(points, x_c, y_c):
    """
    Return ranges, their sum and angles from given points to center.
...
def ranges_in_percentage(ranges, r_sum):
    """
    Calculate the percentage of each given range compared to their sum.
...
def data_between_nearest(points, x_c, y_c):
    """
    Return distance and angle from middle of nearest points to center.
...
def distance_and_angle(dx, dy):
    """
    Calculate and returns distance and angle of values.
